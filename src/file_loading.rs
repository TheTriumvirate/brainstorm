...
fn handle_file_ext(file_ext: &str, data: &[u8], state: &State) -> Result<FileResult, String> {
    match file_ext {
        "bincode" => {
            let vector_field = bincode::deserialize(data)
                .map_err(|e| format!("Failed to parse data: {}", e))?;
            create_providers(vector_field)
        }
        "nhdr" => {
            let string_rep = std::str::from_utf8(data)
                .map_err(|e| format!("Parse error: {}", e))?;
            Ok(FileResult::OptionsFile(reparser::Options::from_header_file(string_rep.lines())))
        },
        "raw" => {
            let options = state.options_file.as_ref()
...
fn create_providers(vectorfield: VectorField) -> Result<FileResult, String> {
    ...
    Ok(FileResult::VectorField((field_provider, gpu_field_provider)))
}
